...
final case class ProductInfo(
    ...
    def withProductId(__v: String): ProductInfo = copy(productId = __v)
    def withNum(__v: Int): ProductInfo = copy(num = __v)
    def withPrice(__v: Double): ProductInfo = copy(price = __v)
    def withRealPrice(__v: Double): ProductInfo = copy(realPrice = __v)
    def withDiscount(__v: Double): ProductInfo = copy(discount = __v)
    def withTotal(__v: Double): ProductInfo = copy(total = __v)
